...
migrateup1:
	migrate -path db/migration -database "postgresql://root:appak@localhost:3333/my_bank?sslmode=disable" -verbose up 1

migratedown:
	migrate -path db/migration -database "postgresql://root:appak@localhost:3333/my_bank?sslmode=disable" -verbose down

migratedown1:
	migrate -path db/migration -database "postgresql://root:appak@localhost:3333/my_bank?sslmode=disable" -verbose down 1
...
mock:
	mockgen -package mockdb -destination db/mock/store.go github.com/appak21/mybank/db/sqlc Store 
